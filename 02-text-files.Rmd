...
file!](https://github.com/ngs-docs/2021-remote-computing-binder/blob/latest/SouthParkData/README.md),
...
and use the arrow keys to move up and down and left and write.
...
Then hit the escape key to go back to "view" mode.
...
* [BBEdit](https://www.barebones.com/products/bbedit/) was beloved by many. Runs on Mac OS X. Not free, but that's ok!
...
So editing is pretty cool, but if you're in a hurry, or want to do something,
short, or need to work with some pretty big files, there are other approaches
you can use. Read on!
...
```
cd ~/
zip south-park.zip SouthParkData/
```

and then make a copy in a new place (just to demonstrate that it all works :) -
```
mkdir new-place/
cd new-place/
...
Note, you can use `unzip -v` to see what's _in_ a zip archive, and selectively unzip specific files by specifying them on the command line like so:
```
