...
This lesson was adopted from [a lesson](https://github.com/ngs-docs/2021-GGG298/tree/latest/Week3-conda_for_software_installation) co-authored by Shannon Joslin for GGG 298 at UC Davis.
...
The 'base' is new and it indicates that conda has been activated in your
account and that you are in the base environment. Read on!
...
operating systems yet (and may neve be).
...
And yes, there are many ways to install and use software!
...
bioinformatics for looking at FASTQ files. The main point is that
it's just some more "non-standard" software that you can install!
...
The default is `base` and anything installed in it is available in all
environments. Additional environments will have the packages in `base` plus
their own commands!
...
What conda does when it switches environments is change the PATH (and
other environment variables) so that the software packages are
searched for in different places.
...
But unfortunately many conda-forge bioconda packages are only
available for OS X and Linux, and not Windows :(. This is because
they are built for Linux.
